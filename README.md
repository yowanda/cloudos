...
- **Remote** — pushes to a CloudOS API server with **incremental diff-sync** as the default. The client tracks a per-VFS logical clock and a "last pushed" watermark; on every mutation (debounced) it sends just `{ since, entries, tombstones }` for paths that advanced since the last round-trip to `POST /api/v1/vfs/changes`, then merges the inverse delta the server returns back into the local tree (last-write-wins by clock, so concurrent local edits win when they're newer). Tombstones cover deletes / renames / moves, are capped to a sliding window in `localStorage`, and persist server-side so deletions propagate to every device. Older servers that don't expose `/changes` (HTTP 404) automatically fall back to the legacy `PUT /api/v1/vfs/snapshot` full-snapshot path. Settings → Backend lets you toggle the mode and view current/last-pushed clock plus pending-delta count.
