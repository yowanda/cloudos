...
- [x] File sync
  ...
  - [x] **Offline support (Service Worker)** — `apps/desktop/public/sw.js` precaches the shell on install and uses a tiered fetch strategy: navigation requests are network-first with a cached `/` fallback, static assets (`/assets/`, JS / CSS / SVG / icons / fonts) are stale-while-revalidate, and API calls (`/api/`, `/auth/`, `/ws/`) are network-only so live data and auth tokens are never served from cache. The cache name is versioned (`cloudos-shell-v2`) so a new deploy purges the old cache on `activate`. Backed by `apps/desktop/public/manifest.webmanifest` (installable PWA: name, theme color, scope, icons) and `icon.svg` / `icon-maskable.svg`. Registration is gated to `import.meta.env.PROD` so the dev server's HMR isn't fighting the SW's caching.
  - [x] **Mobile / responsive shell** — `stores/viewport-store.ts` exposes reactive `isMobile()` (< 640 px) / `isTablet()` (640–1024 px) signals driven by `resize` + `orientationchange` listeners. The shell collapses to a single full-bleed window at < 640 px: `Window.tsx` ignores stored bounds + drag handles + corner resize handles, the floating `Dock` and right-rail `DesktopWidgets` hide, and `Taskbar` strips the per-window text labels, the date line, and the Wi-Fi / Volume placeholders so it fits a 360 px viewport. PWA polish: `manifest.webmanifest` gains `display_override` (`window-controls-overlay` first), `orientation: any`, `launch_handler.client_mode = focus-existing`, and three jump-list `shortcuts` (Files / Terminal / Settings) which are honoured on first paint via a `?launch=<appId>` parser in `App.tsx`. A `<InstallPrompt />` banner listens for `beforeinstallprompt`, surfaces an Install / Not now choice, and suppresses itself for 7 days via `localStorage`.
- [x] File sharing
...
- [x] App Store
  ...
  - [x] Developer portal (submit apps) — windowed `Developer Portal` app (`com.cloudos.devportal`) with three tabs: **Submit** (paste an `AppManifest` JSON, validated server-side, ID must look like `com.example.x`), **My submissions** (status badges, reviewer notes), **Admin review** (only visible to allow-listed admin emails — list pending, approve/reject with mandatory reason on rejection). Backend: `AppSubmission` GORM model, `POST /api/v1/dev/submissions`, `GET /dev/submissions/mine`, admin-gated `GET/POST /dev/admin/submissions[/:id/(approve|reject)]`, plus a public `GET /api/v1/apps/published` projection for the App Store. Approval upserts an `App` row with `Published=true`. Admins are configured via the `ADMIN_EMAILS` env var (comma-separated, case-insensitive).
- [x] AI Assistant
  - [x] Chat UI (windowed app, multi-conversation sidebar, persisted)
  - [x] LLM API integration with **one-click provider presets** in `apps/desktop/src/stores/ai-provider-presets.ts`. The Settings panel exposes a single "Quick start preset" dropdown that auto-fills base URL + model + a curated suggested-models dropdown. Presets covered: **free** (Groq, OpenRouter `:free`, Google Gemini AI Studio, Cerebras, Together AI, Mistral La Plateforme, Ollama local), **paid** (OpenAI, Anthropic), **offline** (Echo mock, custom OpenAI-compatible endpoint). Each preset that requires a key carries a "Get free API key →" button linking straight to the provider's signup / key-management page. API keys are stored only in browser `localStorage`; they are never synced to the CloudOS backend. Suggested-models lists are curated to the free / cheap SKUs the user most likely wants. See `apps/desktop/src/apps/AIAssistant.tsx` and `apps/desktop/src/stores/ai-provider-presets.ts`.
  - [x] Context-aware (can read files, run commands) — rule-based slash-command tool layer in `apps/desktop/src/stores/ai-tools.ts` intercepts user messages before they hit the LLM. Read-only commands: `/help`, `/read <path>`, `/ls [path]`, `/stat <path>`, `/find <pattern>`, `/tree [path] [maxDepth]`, `/storage`, `/clock`, `/conflicts`, `/apps`, `/windows`, `/desktops`, `/whoami`, `/recent`, `/now`. Mutating commands (gated behind Run / Cancel by default): `/write <path> <content>`, `/mkdir <path>`, `/rm [--hard] <path>`, `/mv <src> <dst>`. Works in **every provider mode** (echo / OpenAI / Anthropic / Ollama / OpenAI-compatible) — no API key required, fully offline.
  - [x] **LLM tool-calling** — same `ai-tools.ts` registry exposes `getToolsSchema()` (OpenAI / Ollama shape) and `runTool(name, params, opts)` so the configured LLM can invoke 17 tools directly via OpenAI / OpenAI-compatible / Anthropic / tool-capable Ollama function-calling. Multi-turn loop (LLM → tool → LLM) up to 5 iterations; mutating tools share the same Run / Cancel gate as slash commands. User opts in via Settings → "Let the LLM call CloudOS tools" (off by default). Per-call cards in the chat UI show name + args, results inline once resolved. See [`docs/ASSISTANT_TOOLS.md`](ASSISTANT_TOOLS.md) for the full schema and provider-by-provider wire formats.
...
- [x] **Browser extension** — Chromium / Firefox MV3 extension at [`extension/`](../extension/) that overrides the new-tab page to open the user's CloudOS instance.
  - [x] **MV3 manifest** with `chrome_url_overrides.newtab` pointing at a packaged HTML shell. The shell either redirects to the configured CloudOS URL (default) or embeds it in an iframe — toggleable from the options page since many self-host setups send `X-Frame-Options: deny`.
  - [x] **Options page** with URL editor, mode picker (redirect / iframe), history-bridge toggle, and per-origin host-permission grant button. Settings persist to `chrome.storage.sync` so they roam with the browser profile.
  - [x] **History bridge** — opt-in `chrome.history.search` exposure for the configured CloudOS origin only. When the toggle is on **and** the user has clicked "Grant host permission", a content script is injected into that origin and forwards `cloudos:ext:get-history` `postMessage` requests to the service worker, which answers with `HistoryItem[]` (URL / title / lastVisitTime / visitCount). The service worker double-checks `sender.origin` against the saved CloudOS URL so a mis-configured origin can't read history. Wire format and example client code are documented in [`extension/README.md`](../extension/README.md).
  - [x] **Build pipeline** (`extension/scripts/build.mjs`): esbuild bundles four TypeScript entrypoints (newtab, options, background service worker, content bridge) into ESM, fills `__VERSION__` in the manifest template, rasterises `extension/icons/icon.svg` into 16 / 32 / 48 / 128 PNGs (tries `magick`, then `convert`, then `rsvg-convert`; falls back to a 1×1 transparent PNG with a warning if none are installed), and emits both an unpacked `extension/dist/` and a ready-to-upload `extension/dist/cloudos-extension.zip` archive (built with a dependency-free in-repo zip writer).
  - [x] **Workspace integration** — wired into `pnpm-workspace.yaml`, picked up by `pnpm turbo build`; lint passes under `biome check src/ scripts/` with no errors against an extension-local `biome.json` matching the rest of the frontend (2-space indent, double quotes, recommended rules).
  - [x] **README** with sideload instructions for Chromium-based browsers (`chrome://extensions` → Load unpacked) and Firefox 115+ (`about:debugging` → Load Temporary Add-on), Web Store / AMO release notes, and a permissions rationale.
