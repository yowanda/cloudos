...
- [x] File sync
  ...
  - [ ] Per-entry diff sync / conflict resolution _(stages 1+2a+2b landed; 3-way merge UI still pending)_
    ...
    - **Stage 3 / conflict UI** — _(pending)_ surface entries where local and remote have diverging clocks since their fork point; let the user pick local / remote / merge (3-way) per file. Stage 2b's LWW handles the common case automatically; stage 3 covers the genuine "both sides advanced past the fork concurrently" scenario.
