...
- [x] File sync
  ...
  - [ ] Per-entry diff sync / conflict resolution _(stages 1+2a landed; client adapter wiring + 3-way merge UI still pending)_
    ...
    - **Stage 2b / client adapter** — _(pending)_ wire `vfs/sync.ts` + `vfs/adapters/remote.ts` to send incremental deltas using `entriesChangedSince(lastPushedClock)` + a tombstone log, and apply the response delta back into the VFS via a new `mergeDelta(...)` helper. Settings → Backend toggle to switch between full-snapshot and incremental modes.
    - **Stage 3 / conflict UI** — _(pending)_ surface entries where local and remote have diverging clocks since their fork point; let the user pick local / remote / merge (3-way) per file.
